...
Cross-links coupled with mass spectrometry (CXMS) is an upcoming and promising
...
information might be useful in complementing the CXMS based distance
restraints, since the allowed distance ranges for the latter can be relatively
wide (up to 30Å \cite[Merkley2014]).

Inclusion of CXMS based distance restraints has already been shown to
...
with a to be determined AIC cutoff based on the analysis that was described in
the previous section . Per HADDOCK run 1000 it0-structures were written to
file, using 5 trials per file combined with 180° rotated solutions, resulting in 10,000 sampled
solutions (\m{1000 \times 5 \times 2}), of which the 200 best scoring solutions
were subjected to the semi-flexible refinement (it1 and itw) (default settings
of the server). The solutions were analyzed by calculating the ligand-RMSD
...
residue. This shows for example that for all complexes, when using 7 cross-links,
for residues that have an AIC \m{≥} 1.0 the precision is approximately 60\%, i.e.
60\% of all residues with a AIC \m{≥} 1.0 are true interface residues; the recall
at 1.0 AIC is around 40\%, meaning that 40\% of all true interface residues are
still retained in the set of residues satisfying the AIC cutoff condition.

\placefigure[page][fig:precision-recall]
...
complexes this increases even to 60, 85 and 90\%, while for Medium and
...
\inb[fig:benchmark-rosetta], respectively, in terms of the structure with the
