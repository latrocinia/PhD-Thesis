% math definitions
\define[0]\Cmax{{\bf C}_{\text{max}}}
\define[0]\Imin{{\bf I}_{\text{min}}}
\define[0]\rvec{\vec{r}}
\define[0]\rvecF{\rvec_{\text{F}}}
\define[0]\rvecS{{\rvec}_{\text{S}}}
\define[0]\rveccomS{{\rvec}_{\text{comS}}}
\define[0]\boldL{{\bf L}}
\define[0]\boldC{{\bf C}}
\define[0]\boldI{{\bf I}}
\define[0]\boldS{{\bf S}}
\define[0]\boldF{{\bf F}}
\define[0]\boldA{{\bf A}}
\define[0]\redais{{\bf A}_{R, \text{red.}}}
\define[0]\dmin{d_{\text{min}}}
\define[0]\dmax{d_{\text{max}}}
...
fter every translational scan, all conformations that comply with each
estraint are determined. Next, disvis counts the number of accessible
tatescomplexes consistent with each number of restraints, as well as which
estraints are violated. This is repeated until the rotational sampling density
eaches a pre-set value (default 9.72\Deg, 7416 orientations). During the
otational search, disvis stores the maximum number of consistent restraints
ound at every scanned position of the ligand’s center of mass, which
ltimately results in a discrete ‘density’ map. The output thus consists of the
um of accessible interaction statescomplexes complying with each number of
estraints, a percentage of how often each restraint is violated, and a
iscrete-valued density map.

e implemented DisVis in Python2.7, using the OpenCL framework to offload
omputations to the GPU. The code can be downloaded freely from
ttps://github.com/haddocking/disvis together with documentation and examples.
...
FFT-techniques as used originally in \cite[authoryear][Katchalski-Katzir1992].
...
as a function of rotation $R$ as follows
...
where the cross-correlation theorem has been used to calculate ${\bf C}$ and
${\bf I}$, the spaces that represent the volume of clashes and interactions at
every grid position in \Angstrom\high{3}, respectively; $\mathcal{F}$ and
$\mathcal{F}^{-1}$ represent the FFT operator and its inverse, respectively; \high{*}
is the complex conjugate operator, and $\times$ the elementwise multiplication
operator; ${\bf S}$ is the shape of the scanning chain, and ${\bf
F}_{\text{core}}$ and ${\bf F}_{\text{inter}}$ are the core and interaction
...
\NC \text{if}\ \boldC_R \left(\rvec \right) \leq \Cmax\ \text{and}\ \boldI_R\left(\rvec \right) \geq \Imin:  \MC 1 \NR
\NC \text{else:} \MC 0 \NR
\stopmathcases
\stopformula
...
\cite[authoryear][Karney2007], which include a weight factor for every rotation
to average out redundancy. The total number of accessible states \m{N_A} is
thus given by

\placeformula[eq:accessible-states]
\startformula
N_A = \sum_{\bf P} w_R \sum_{x, y, z} {\bf A}_R\left( x, y, z \right)
\stopformula

where \m{w_R} is the weight factor for the specific orientation/rotation, the first
summation is over all rotations \m{\bf P}, and the second summation over all grid
coordinates.
...
of the center of this sphere \m{\rvec_c}, which is given by
...
\NC \text{if}\ \dmin \leq |\rvec - \rvec_c | \leq \dmax: \NC 1 \NR
\NC \NC \NR
\NC \text{else}: \NC 0\NR
\stopmathcases
\stopformula
...
Note that the function describing \m{\boldL_R} can be freely chosen, under the
...
where the summation is over all distance restraints \m{N_d} and \m{\boldL_{R,n}} is the space
conforming to distant restraint \m{n}. The value found at a specific coordinate in
...
%
%would put a restraint between the CA-atom of residue 18 of chain A of the fixed
...
%of distance restraints, the number of occurrences that 𝑨!,  red. is equal to
...
%restraint is violated for complexes consistent with at least 𝑁 restraints (e.g.
...
%
%where 𝑣! is the column average of the violation matrix of restraint 𝑖, and 𝑣
%and 𝜎 are the average and standard deviation of the violation matrix. Disvis
%reports restraints with a z-score higher than 1.0 explicitly.
%
