...
We implemented DisVis in Python2.7, using the OpenCL framework to offload
computations to the GPU. The code can be downloaded freely from
https://github.com/haddocking/disvis together with documentation and examples.
...
restraint is violated for complexes consistent with at least \m{N} restraints (e.g.
...
Python bindings (https://github.com/geggo/gpyfft) to calculate the FFT’s.
Computations were performed on AMD Opteron 6344 CPU processors and on an AMD
Radeon HD 7730M and NVIDIA GeForce GTX 680 GPU. 
...
\cite[Merkley2014]. The restraints used are shown in Table S1. The input files
are included in the DisVis source code. 

Two disvis runs were performed using a rotational sampling density of 5.27\Deg\ 
(53256 orientations) and 9.72\Deg\ (7416 orientations) with a grid spacing of
1\Angstrom\ and 2\Angstrom, respectively. All parameters were left to there
default values (interaction radius 3\Angstrom, minimum required volume of
interaction 300\Angstrom\high{3} and maximum allowed volume of clashes
200\Angstrom\high{3}). The amount of accessible complexes consistent with each
number of cross-links is shown in Table S2 and S4, and the relative occurrence
of restraint violations in Table S3 and S5 for the fine and coarse run,
respectively.
...
from \cite[authoryear][Leitner2014] Dataset S1 (Table S6), which consist of 4
ADH and 3 zero-length ZL cross-links. The maximum ADH- and ZL linker length
were set to 23\Angstrom\ and 26\Angstrom, respectively, since 95\% of all found
distances in a benchmark were smaller. All input files are included in the
DisVis source code. 
...
spacing of 1\Angstrom\ and 2\Angstrom, respectively, with default parameter
values. The sum of accessible complexes consistent with each number of
restraints is shown in Table S7 and S9, and their normalized restraint
violation occurrence in Table S8 and S10. 
