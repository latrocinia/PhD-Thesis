...
rigid body fitting software can be used (e.g. \cite[authoryear][Hoang2013]) to
...
Once the centroids have been determined the docking can start. First the
chains are separated in space at an approximate minimal distance of 25\Angstrom\ from
...
energy (see Experimental Procedures, Eq.
\in[eq:haddock-em-it0]-\in[eq:haddock-em-itw]).  The 400 best scoring models
are then refined using the standard HADDOCK refinement protocol with an
additional correlation-based potential to further fit the chains into the
density, while reckoning with the energetics of the system. 
...
except for the smallest 2OOB complex when using 15 and 20\Angstrom\ resolution data.
The average i-RMSD improvement is 1.2, 1.5 and 1.6\Angstrom\ when using 20, 15 and
10\Angstrom\ data, respectively.  Even for complexes for which no acceptable solutions
were generated, there is a considerable increase of quality, e.g. the i-RMSD of
the hard 1JMO complex decreases from 6.13 for HADDOCK-CM to 4.66, 4.35 and
4.51\Angstrom\ when using 20, 15 and 10\Angstrom\ data, respectively. 
...
\in{Figure}[fig:lcc-scores]), we observe a funnel shape for most complexes,
...
(EMD-2355, \cite[authoryear][Dauden2013]).  As in the previous cases, the
